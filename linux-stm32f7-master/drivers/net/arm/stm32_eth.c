...
#if defined(CONFIG_ARCH_STM32)
#define STM32_RCC_AHB1RSTR_ETHMAC	(1 << 25)

static void stm32_params_setup(struct stm32_eth_priv *stm);

/*
 * MACMIIAR.CR value for the current HCLK (RM0385 table for MDC clock)
 */
static u32 stm32_eth_mdio_cr(void)
{
	unsigned int hclk = stm32_clock_get(CLOCK_HCLK);
...
static int stm32_eth_hw_start(struct net_device *dev)
{
...
#if defined(CONFIG_ARCH_STM32)
	/*
	 * Bring the MAC up from a known state, the way the boot loader does,
	 * rather than inheriting whatever it left behind: a peripheral reset
	 * pulse, then the DMA software reset (which needs the PHY clock
	 * present, so this runs after the PHY has been started). On the
	 * STM32F7-DISCO (RMII) the MAC inherited from U-Boot accepted frames
	 * for transmission but never drove the RMII TX pins.
	 */
	STM32_RCC->ahb1rstr |= STM32_RCC_AHB1RSTR_ETHMAC;
	STM32_RCC->ahb1rstr &= ~STM32_RCC_AHB1RSTR_ETHMAC;
	...
	if (i == 100) {
		printk(STM32_INFO ": MAC reset timeout (no PHY clock?)\n");
		rv = -EBUSY;
		goto out;
	}

	/*
	 * The reset cleared the MDIO clock range; the PHY is polled through
	 * it from now on. RM0385: HCLK 150..216 MHz -> /102
	 */
	stm->regs->macmiiar = stm32_eth_mdio_cr() << STM32_MAC_MIIAR_CR_BIT;

	/* ... and the MAC configuration (duplex/speed from the PHY) */
...
#else
		/*
		 * The ring does not fit in the 64 KB DTCM on STM32F7, so the
		 * upper buffers live in SRAM1, which the Cortex-M7 D-cache
		 * does cache. Drop any lines the CPU still holds from the
		 * previous frame in this buffer before copying it out.
		 */
		dmac_inv_range(stm->rx_skb[idx]->data,
			       stm->rx_skb[idx]->data + len);
#endif
...
#else
	/*
	 * Copy into SRAM and free. The copy must reach memory before the
	 * DMA fetches it: the TX buffers sit in cacheable SRAM1 (see the
	 * RX path), and a frame left in the write-back D-cache goes out on
	 * the wire as whatever was in SRAM before it.
	 */
	skb_copy_from_linear_data(skb, stm->tx_skb[idx]->data, len);
	stm->tx_skb[idx]->len = len;
	dmac_clean_range(stm->tx_skb[idx]->data, stm->tx_skb[idx]->data + len);
	dev_kfree_skb(skb);
#endif

	stm->tx_bd[idx].ctrl  = len;
#ifndef STM32_SRAM
	/*
