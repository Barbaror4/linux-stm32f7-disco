 ...
 */
/*
 * GPIOx_OTYPER is one bit per pin: 0 = push-pull, 1 = open-drain. The F7
 * values used to be 0x02/0x12, which is wrong for a 1-bit field: applied as
 * "otyper |= otype << pin" they set the NEXT pin's bit instead. Configuring
 * PG12 (LCD_B4) for the LTDC role that way forced PG13 (RMII_TXD0) to
 * open-drain, so the Ethernet MAC could never drive TXD0 high and no frame
 * ever left the board (RX, all inputs, was unaffected).
 */
#define STM32F2_GPIO_OTYPE_PP	0x00
#define STM32F2_GPIO_OTYPE_OD	0x01
#define STM32F7_GPIO_OTYPE_PP	0x00
#define STM32F7_GPIO_OTYPE_OD	0x01
	...
	struct stm32f2_gpio_dsc *dsc, enum stm32f2_gpio_role role)
{
	...
	case STM32F2_GPIO_ROLE_SDIO:
		/*
		 * The SD card lines need pull-ups: the boards we run on do
		 * not fit external ones, and CMD/DATx are open at the card
		 * side until the card drives them.
		 */
...
#error		IOMUX for STM32 SPI6 undefined
		...
		do {
			...
			/*
			 * Card detect. On the STM32F7-DISCO the microSD
			 * socket pulls PC13 low while a card is seated;
			 * stm32_sdcard_init() reads it back through
			 * mmc_card_detect().
			 */
			if (platform == PLATFORM_STM32_STM32F7_DISCO) {
				struct stm32f2_gpio_dsc cd_gpio = {2, 13};

				stm32f2_gpio_config(&cd_gpio,
						    STM32F2_GPIO_ROLE_IN_PUP);
			}
		} while (0);
		...
		do {
			...
			/*
			 * On the STM32F746G-DISCO, VBUS of CN13 (USB OTG FS
			 * Micro-AB) comes from the STMPS2151STR power switch,
			 * whose enable input is ACTIVE LOW (the ...41 variant
			 * is the active-high one). PD5 (OTG_FS_PowerSwitchOn)
			 * therefore has to be driven low to power the port;
			 * LD5 lights while VBUS is on.
			 */
			if (platform == PLATFORM_STM32_STM32F7_DISCO) {
				struct stm32f2_gpio_dsc vbus_gpio = {3, 5};
				volatile struct stm32f2_gpio_regs *gpio_regs;

				stm32f2_gpio_config(&vbus_gpio,
						    STM32F2_GPIO_ROLE_OUT);
				gpio_regs = (volatile struct stm32f2_gpio_regs *)stm32_gpio_base[3];
				gpio_regs->odr &= ~(1 << 5);
			}
		} while (0);
