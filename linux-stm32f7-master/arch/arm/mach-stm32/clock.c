...
/*
 * On STM32F7 the PLLSAI also feeds the 48 MHz clock domain (USB OTG FS,
 * SDMMC, RNG) through its P output, selected with DCKCFGR2.CK48MSEL. The
 * main PLL cannot provide it: the 200 MHz SYSCLK needs a 400 MHz VCO, and
 * no PLLQ divides 400 MHz down to 48 MHz (PLLQ=8 gives 50 MHz, 4% off the
 * +/-0.25% the USB full-speed PHY requires). So the PLLSAI VCO is fixed at
 * 288 MHz: /6 on P gives exactly 48 MHz, and /4 on R followed by /8 in
 * DCKCFGR1 gives the same 9 MHz LTDC pixel clock as before.
 */
#define STM32F7_PLLSAI_VCO_HZ	288000000
#define STM32F7_PLLSAI_P_DIV6	(2 << 16)	/* PLLSAIP field: 10 = /6 */
#define STM32F7_PLLSAI_R	4
#define STM32F7_PLLSAI_DIVR	RCC_PLLSAIDivR_Div8
#define STM32F7_CK48_HZ		48000000
#define RCC_DCKCFGR2_CK48MSEL	(1 << 27)	/* 1: 48 MHz clock from PLLSAIP */
#endif
...
#ifdef CONFIG_ARCH_STM32F7
	/* VCO is pinned by the 48 MHz consumer, see above */
	sai_n = STM32F7_PLLSAI_VCO_HZ / parent_rate;
	sai_r = STM32F7_PLLSAI_R;
	sai_div_r = STM32F7_PLLSAI_DIVR;
	pllsaicfgr = (sai_n << 6) | STM32F7_PLLSAI_P_DIV6 | (sai_q << 24) |
		     (sai_r << 28);

	/*
	 * Already running with this configuration (the USB/SD clock set-up
	 * in stm32_clock_init() got here first): leave it alone, a restart
	 * would glitch every consumer of the 48 MHz domain.
	 */
	if ((STM32_RCC->cr & RCC_CR_PLLSAION) &&
	    STM32_RCC->pllsaicfgr == pllsaicfgr &&
	    (STM32_RCC->dckcfgr & RCC_DCKCFGR_PLLSAIDIVR) == sai_div_r)
		return;
#else
	sai_r = 3;
	sai_div_r = RCC_PLLSAIDivR_Div8;

	/* Calculate N to match the requested rate */
	sai_n = clk->rate * sai_r * 8 / parent_rate;
	pllsaicfgr = (sai_n << 6) | (sai_q << 24) | (sai_r << 28);
#endif
...
#ifdef CONFIG_ARCH_STM32F7
	/*
	 * Bring up the PLLSAI now and route its 48 MHz P output to the
	 * CK48 domain, so that USB OTG FS and SDMMC get a real 48 MHz
	 * whether or not the LCD driver is enabled.
	 */
	sai_r_clk_enable(&clk_sai_r);
	STM32_RCC->dckcfgr2 |= RCC_DCKCFGR2_CK48MSEL;
	clk_mci.rate = STM32F7_CK48_HZ;
#endif
