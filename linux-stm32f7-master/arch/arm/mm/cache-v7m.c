...
void v7m_dma_unmap_area(const void *start, size_t size, int dir)
{
	...
	/*
	 * Drop the cache lines covering an inbound buffer once the device
	 * has filled it: the Cortex-M7 can refill them behind our back
	 * while the transfer is in flight, which would leave the CPU
	 * reading pre-DMA data. The lines were cleaned in
	 * v7m_dma_map_area(), so nothing dirty is lost here.
	 *
	 * Note this used to test for DMA_TO_DEVICE, which invalidated
	 * exactly the direction that does not need it and skipped the one
	 * that does.
	 */
